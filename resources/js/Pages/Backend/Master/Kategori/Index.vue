...
const confirmDelete = (item) => {
    confirm.require({
        message: `Apakah Anda yakin ingin menghapus kategori "${item.label}"?`,
...
<template>
    ...
    <AuthenticatedLayout>
                ...
                <div class="max-w-full w-full bg-white rounded-lg shadow-sm dark:bg-gray-800 p-4 md:p-6 dark:text-gray-100">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4 md:gap-0">
                        <div class="flex flex-col sm:flex-row gap-2 md:gap-4">
                            <InputText size="small" v-model="search" placeholder="Cari kategori..." class="border block rounded w-full sm:w-64 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                        </div>
                    ...
                    >
                        ...
                        <Column field="label" header="Nama Kategori" sortable>
                            <template #body="{ data }">
                        ...
                        <Column field="permohonans_count" header="Jumlah Permohonan" sortable class="w-40 text-center">
                            <template #body="{ data }">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                                    {{ data.permohonans_count || 0 }}
                                </span>
                            </template>
                        </Column>
        ...
        <!-- Modal Create/Edit -->
        <Dialog v-model:visible="showModal" :header="editMode ? 'Edit Kategori' : 'Tambah Kategori'" modal :style="{ width: '450px' }">
            <form @submit.prevent="submitForm" class="space-y-4">
                <div class="flex flex-col gap-2">
                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Nama Kategori</label>
                    <InputText v-model="form.label" placeholder="Masukkan nama kategori" class="w-full" autofocus />
